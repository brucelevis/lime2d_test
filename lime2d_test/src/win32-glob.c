...
{
    int len;
	file_entry* cur = (file_entry*)malloc(sizeof(file_entry));
    if (!cur)
    {
        return ENOMEM;
    }
...
entry_comparer(
    ...
    const void* pv2)
{
	const file_entry* const * pe1 = (const file_entry* const *)pv1;
	const file_entry* const * pe2 = (const file_entry* const *)pv2;
    ...
    glob_t *pglob)
{
    ...
    if (err == 0)
    {
		pglob->gl_pathv = (char **)malloc((entries + 1) * sizeof(char*));
        if (pglob->gl_pathv)
        {
